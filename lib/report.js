...
function renderText(result) {
  ...
  if (result.operationalError) {
    lines.push(`ERROR: ${result.operationalError}`);
...
// which isn't true of optional uriBaseId support. Both real callers
// (bin/cws-gate.js, action/index.js) call renderSarif(result) with a single
// argument, so default CLI/Action output keeps package-relative uris.
function renderSarif(result, rootPrefix) {
  ...
  // given, so the uri below reduces to plain f.path unchanged.
  const normalizedPrefix = rootPrefix ? rootPrefix.replace(/\\/g, '/').replace(/^\/+/, '').replace(/\/+$/, '') : '';

  const sarifResults = result.findings.map((f) => {
    const uri = normalizedPrefix ? `${normalizedPrefix}/${f.path}` : f.path;
    return {
      ruleId: f.ruleId,
